...
export const useUpdateUser = () => {
    ...
    return useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            console.log(`User ${variables.userId} updated successfully!`);
        },
...
//         queryClient.invalidateQueries(["users"]);
//         queryClient.invalidateQueries(["user", variables.userId]);
//         console.log(`Bank details for user ${variables.userId} updated successfully!`);
//       },
//         onError: (error, variables) => {
//             console.error(
//                 `Error updating user: ${error.response?.data?.message || error.message}`
...
export const useUpdateBankDetails = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({userId, bankDetails}) => updateBankDetails({userId, bankDetails}),
        onSuccess: (data, variables) => {
            queryClient.invalidateQueries(["users"]);
            queryClient.invalidateQueries(["user", variables.userId]);
            console.log(`Bank details for user ${variables.userId} updated successfully!`);
        ...
        onError: (error, variables) => {
            console.error(
                `Error updating bank details: ${error.response?.data?.message || error.message}`
            );
            throw error;
        },
    });
